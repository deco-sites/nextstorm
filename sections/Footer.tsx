        ...
        <div class="flex flex-col gap-6 justify-between lg:flex-row">
          <div class="flex flex-col gap-2">
            ...
            {textBelowLogo?.map((text) => (
              text.href ?
                <a
                  class="block hover:text-[#93C50C] transition-colors duration-300 link no-underline py-1 whitespace-nowrap"
                  href={text.href}
                >
                  {text.label}
                </a>
                :
                <span
                  class="block no-underline py-1 whitespace-nowrap"
                  href={text.href}
                >
                  {text.label}
                </span>
          ...
          <div class="flex gap-9 lg:gap-0 lg:justify-evenly lg:w-full">
            {links?.map((link) => (
              <div>
                {link.title && <h4 class="font-semibold mb-4 hover:text-[#93C50C] transition-colors duration-300">{link.title}</h4>}
                {link.items?.map((item) => (
                  item.href ?
                    <a
                      class="block hover:text-[#93C50C] transition-colors duration-300 link no-underline py-1"
                      href={item.href}
                      dangerouslySetInnerHTML={{ __html: item.label }}
                    />
                    :
                    <span
                      class="block no-underline py-1"
                      href={item.href}
                      dangerouslySetInnerHTML={{ __html: item.label }}
                    />
                ))}
          ...
          <div class="flex flex-col gap-1">
            <span class="text-xs">Siga-nos nas redes sociais</span>
